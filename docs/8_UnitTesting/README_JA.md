...
[前へ - MVC 実装 + Vision](../7_ImplementMVC/README_JA.md) | [次へ - Custom Agent](../9_CustomAgent/README_JA.md)
...
```
app/MeowWorld に対する xUnit テストプロジェクトを app/MeowWorld.Tests として作成してください。
...
> **Note:** `/tests` は Ask モードでも使えます。ただし Agent モードであれば、生成されたテストファイルの配置・ビルド・実行まで自動で行います。
