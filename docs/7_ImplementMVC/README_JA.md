...
[前へ - DB レイヤー実装](../6_ImplementDBLayer/README_JA.md) | [次へ - ユニットテスト](../8_UnitTesting/README_JA.md)
...
Copilot Chat を **Agent モード**にして、`/` を入力し `create-crud-controller` を選択します：
...
[Microsoft Designer](https://designer.microsoft.com/) を添付し、以下のようなプロンプトで画像を生成します：

```
添付の画像はスケッチです。参考にしてシンプルでかわいい猫のロゴを作成してください。
```
...
本リポジトリには事前に生成済みのロゴが含まれています。以下のコマンドでコピーするだけで使えます：

```bash
cp docs/assets/logo.png app/MeowWorld/wwwroot/images/logo.png
```
...
2. チャット入力欄の **クリップアイコン（📎）** をクリックし、以下の画像を添付します：
   ...
   > **画像が見つからない場合:** 講師に確認するか、下部の「テキストベースの代替手段」を使ってください。
